...
const Seat = ({row, col, selectedSeat, seatSpecificPrice, blockedSeat, notASeat, bookedSeat, generalSeat, reservedSeat, sociallyDistancedSeat, handleMouseClicked}) => {
    return (
        <div
            ...
            className={`bus-seat seat-gap ${generalSeat && selectedSeat && 'g-clicked'} ${reservedSeat && selectedSeat && 'r-clicked'} ${sociallyDistancedSeat && selectedSeat && 'sd-clicked'} ${!notASeat && 'not-a-seat'} ${blockedSeat ? 'b-outline not-allowed-cursor' : bookedSeat ? 'b-outline not-allowed-cursor' : generalSeat ? 'g-outline allowed-cursor'  : reservedSeat ? 'r-outline allowed-cursor' : sociallyDistancedSeat ? 'sd-outline allowed-cursor' : ''}`}
            // className={`bus-seat ${col === 1 ? 'passage-gap' : 'seat-gap'}`}
        >
            <div className={`seat-top ${bookedSeat && 'st-outline'}`}>
            ...
            <div className={`seat-bottom ${blockedSeat ? 'blocked' : bookedSeat ? 'blocked' : generalSeat ? 'general' : reservedSeat ? 'reserved' : sociallyDistancedSeat ? 'socially-distanced' : ''}`}></div>
